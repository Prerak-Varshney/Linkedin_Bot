import { buttonClick, buttonClickWithIndex } from '../utils/handleButtonClickFunction.js';
import { AUTH_JOBTITLE, AUTH_LOCATION } from '../config/env.js';
import handleGoto from '../config/handleGoto.js';
...
const searchJobs = async (page) => {
  try {
    ...
    //Handle URL
    await handleGoto(page, 'https://www.linkedin.com/jobs');
    ...
    //Apply intern filter
    await buttonClick(page, '#experience-1');
    console.log('Applied "Experience level" filter');
    
    // Click on apply experience level filter
    await buttonClickWithIndex(page, '.artdeco-button--primary', 1);
    console.log('Applied "Experience level" filter');
    ...
    //click on easy apply
    await buttonClickWithIndex(page, '.artdeco-button--primary', 4);
